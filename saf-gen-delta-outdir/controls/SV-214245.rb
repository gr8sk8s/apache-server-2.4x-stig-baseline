...
allow clients to modify unauthorized files on the web server.'
  desc 'check', 'In a command line, run "httpd -M | sort" to view a list of installed
modules.
    ...
    dav_fs_module
    dav_lock_module'
  desc 'fix', %q(Determine where the "dav" modules are located by running the following
command:
